...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var result2 = from x in debtors
                          where (DateTime.Now.Year - x.BirthDay.Year) >= 26 || (DateTime.Now.Year - x.BirthDay.Year) <= 36
                          select x;
            ...
            //4) Butov adi 15 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
            var result4 = from x in debtors
                          where x.FullName.Length > 15 && x.Phone.Where(y => y.Equals('7')).Count() >= 2
                          select x;
            ...
            var result6 = from x in debtors
                          where x.Debt > debtors.Sum(y => y.Debt) /debtors.Count
